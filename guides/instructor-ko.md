# Swarm_ID — 교수자용 가이드 (한국어)

수업용 Swarm_ID 준비를 위한 단계별 안내서입니다. 각 단계는
...
![1단계 — 로그인](./screenshots/instructor-ko/01-login.png)

**https://swarmid.vercel.app** 에 접속해 교수자 계정으로 로그인합니다
...
![2단계 — 스튜디오 진입](./screenshots/instructor-ko/02-studio-landing.png)

스튜디오의 **케이스 네트워크** 뷰로 진입합니다. 왼쪽 사이드바에는
네비게이션(네트워크 · 관점 · 트레이드오프 · 실험 공간 · 리포트)과
...
![3단계 — 케이스 라이브러리](./screenshots/instructor-ko/03-case-library.png)

**시작 패널**을 스크롤해 **강의 케이스** 목록을
...
![4단계 — 케이스 열기](./screenshots/instructor-ko/04-case-open.png)

수업에 사용할 케이스(예: *5학년 온라인 디지털 리터러시 모듈*)의
**케이스 열기** 버튼을 누르면 캔버스가 재렌더됩니다:

- 케이스 제목을 담은 중앙 **코어** 노드
- 코어를 둘러싼 이해관계자 노드(교사·학생·에듀테크·행정·접근성)
...
![5단계 — 설명요약 업로드](./screenshots/instructor-ko/05-upload-brief.png)

이번 수업에 새 케이스가 필요하면, 시작 패널의
...
도구·정책·워크로드), 최소 한 가지 쟁점 지점(예: "저작 정책 vs
AI 보조 드래프팅"). 쟁점이 많이 인코딩될수록 스웜이 그리는 **이견
엣지**가 더 풍부해집니다.
...
![6단계 — 보드 설정](./screenshots/instructor-ko/06-board-settings.png)
...
![7단계 — 관점](./screenshots/instructor-ko/07-perspectives.png)

왼쪽 내비게이션의 **관점**을 클릭합니다. 이해관계자별 상위 충돌
요약과 정량적 쟁점 점수가 표시됩니다. 학생이 스스로 스웜 라운드를
...
![8단계 — 트레이드오프](./screenshots/instructor-ko/08-trade-offs.png)
...
![9단계 — 실험 공간](./screenshots/instructor-ko/09-sandbox.png)

**실험 공간**에서는 사전 정의된 시나리오(*예산 · 접근성 · 워크로드*)
를 적용해 지표가 어떻게 재분배되는지 관찰할 수 있습니다. **자율
반복**을 켜면 시스템이 안정 상태에 이를 때까지 충돌을 계속
셔플합니다.

"만약에" 연습에 가장 적합한 뷰입니다. 시나리오 적용 → 학생이
쟁점이 어디로 이동할지 예측 → 공개 → 토의 순서.

---

## 10단계 · 리포트 & 내보내기

![10단계 — 리포트](./screenshots/instructor-ko/10-report.png)
...
3. 교수자 본인이 수업용 케이스를 최소 한 번은 열어 봄 (캐시 워밍 +
   Gemini 쿼터 문제 선검출).
